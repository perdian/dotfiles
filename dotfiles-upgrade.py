...
print "Resolved environments: " + str(active_environments)

# Lookup the root directory of the dotfiles and start the installation
environments_root_directory = os.path.join(os.path.dirname(os.path.abspath(__file__)), "environments")
print "Installing dotfiles from root directory: " + environments_root_directory
...
for active_environment in active_environments:
    active_environment_directory = os.path.join(environments_root_directory, active_environment);
    if os.path.exists(active_environment_directory):

        print "Installing dotfiles from environment directory: " + active_environment_directory;
        for root, dir_names, file_names in os.walk(active_environment_directory):
            for source_file_name in fnmatch.filter(file_names + dir_names, '*.symlink'):
                ...
                if os.path.exists(target_file_resolved) and source_file_resolved == target_file_resolved:
                    ...
                    print "Target '" + str(target_file) + " already symlinked to '" + str(source_file_resolved) + "'"
