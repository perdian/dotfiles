...
An additional profile will be activated that resides outside the dotfiles installation folder. This will allow you to have machine-specific configurations that are not part of the dotfiles repository. The addtional profile will be sourced from the folder `~/.dotfiles-local-profile/` if the folder exists.
